/**
 * Marching Cubes — extract a triangle mesh isosurface from a scalar field.
 *
 * Used to convert a final SIMP density grid (in [0,1]) into a manufacturable
 * surface mesh. The classic Bourke / Lorensen-Cline edge & triangle tables are
 * embedded below; output positions are in world space using the supplied
 * `origin` and `voxelSize`.
 *
 * The implementation is dependency-free and works in any JS runtime.
 */
import type { RawMesh, V3 } from './types';

export interface DensityField {
  /** Scalar values, layout: i + nx*(j + ny*k). */
  data: Float32Array;
  /** Grid dimensions [nx, ny, nz]. */
  dims: [number, number, number];
  /** World-space origin of the (0,0,0) voxel center. */
  origin: V3;
  /** Uniform voxel spacing (mm). */
  voxelSize: number;
}

export interface MarchingCubesOptions {
  /** Iso-level to extract. Default 0.5. */
  isoLevel?: number;
  /** Weld coincident vertices via spatial hash. Default true. */
  weld?: boolean;
}

// ─────────────────────────────────────────────────────────────────────────
// Edge / triangle tables (standard Bourke tables)
// ─────────────────────────────────────────────────────────────────────────
// prettier-ignore
...
// 12 cube edges → corner index pairs
...
// Cube corner offsets (i,j,k) — order matches Bourke's convention
...
/**
 * Run marching cubes over a scalar field and return a `RawMesh`.
 */
export function marchingCubes(
  ...
  const ox = field.origin[0];
  const oy = field.origin[1];
  const oz = field.origin[2];
  ...
  // Spatial-hash welding (round to 1e-5 mm)
  const vertexMap = weld ? new Map<string, number>() : null;
    ...
    for (let j = 0; j < ny - 1; j++) {
      for (let i = 0; i < nx - 1; i++) {
        ...
        for (let c = 0; c < 8; c++) {
          ...
          corner[c] = data[idx(i + o[0], j + o[1], k + o[2])];
        }

        let cubeIndex = 0;
        if (corner[0] < iso) cubeIndex |= 1;
        if (corner[1] < iso) cubeIndex |= 2;
        if (corner[2] < iso) cubeIndex |= 4;
        if (corner[3] < iso) cubeIndex |= 8;
        if (corner[4] < iso) cubeIndex |= 16;
        if (corner[5] < iso) cubeIndex |= 32;
        if (corner[6] < iso) cubeIndex |= 64;
        if (corner[7] < iso) cubeIndex |= 128;

        const edgeMask = EDGE_TABLE[cubeIndex];
        if (edgeMask === 0) continue;

        // Compute interpolated vertices for active edges
        for (let e = 0; e < 12; e++) {
          ...
          const x = ox + (i + oa[0] + t * (ob[0] - oa[0])) * h;
          const y = oy + (j + oa[1] + t * (ob[1] - oa[1])) * h;
          const z = oz + (k + oa[2] + t * (ob[2] - oa[2])) * h;
          edgeVerts[e] = emitVertex(x, y, z);
        }
...
/**
 * Convenience: convert a SIMP `TopoProposal`-style density grid into a mesh.
 */
export function meshFromDensity(
  ...
  return marchingCubes({ data: density, dims, origin, voxelSize }, options);
}
